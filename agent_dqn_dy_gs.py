...
class AgentDQN(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        self.finetuned_path = "/content/drive/MyDrive/DialogDQN-Variants/fine_movie40graphsage-n32_model_adj_recon.pt"
        self.pretrained_path = "/content/drive/MyDrive/DialogDQN-Variants/movie40graphsage-n32_node_embeddings_adj_recon.pt"

        
    def grl_path(self):
        if os.path.exists(self.finetuned_path):
            #print("⚡ Fine-tuned GraphSAGE model found. Loading...")
            path= self.finetuned_path
        else:
        ...
        '''self.finetuned_path = "/content/drive/MyDrive/DialogDQN-Variants/movie40-32n_node2vec_finetuned.csv"
        self.embedding_path = "/content/drive/MyDrive/DialogDQN-Variants/movie40-32n_node2vec_embeddings.csv"
        
        self.node2vec_embeddings = self.load_node2vec_embeddings()


    def load_node2vec_embeddings(self):
        # Try to use finetuned if exists, else fall back
        if os.path.exists(self.finetuned_path):
            print("✅ Using fine-tuned Node2Vec embeddings.")
            path = self.finetuned_path
        else:
            print("⚠️ Fine-tuned file not found. Using original Node2Vec embeddings.")
            path = self.embedding_path

        return path  # or appropriate fallback shape'''
    ...
    def prepare_state_representation(self, state):
        ...
        '''Topol_rep = self.get_graph_embedding(
                    ...
                    grl_path='/content/drive/MyDrive/DialogDQN-Variants/movie40graphsage-n32_node_embeddings_adj_recon.pt'
                    )'''
        #print(self.node2vec_embeddings)
        '''Topol_rep = self.get_graph_embedding(
            contextual_rep=contextual_rep,
            kmeans_path='/content/drive/MyDrive/DialogDQN-Variants/kmeans_modelMovie_k40.joblib',
            #node2vec_csv_path='/content/drive/MyDrive/DialogDQN-Variants/movie40-32n_node2vec_embeddings.csv'
            grl_path= self.node2vec_embeddings
            )'''
        Topol_rep = self.get_graph_embedding(
            contextual_rep=contextual_rep,
            kmeans_path='/content/drive/MyDrive/DialogDQN-Variants/kmeans_modelMovie_k40.joblib',
            grl_path= self.grl_path()
            )
    ...
    def get_graph_embedding(self, contextual_rep, kmeans_path, grl_path):
        ...
        if self.grl_path().endswith('.csv'):       
            embeddings_df = pd.read_csv(self.grl_path())            
            ...
            embeddings_tensor = torch.load(self.grl_path())  # .pt or .pth
            #print("embeddings_tensor", embeddings_tensor.keys())
            embeddings_np = embeddings_tensor.detach()
    ...
    def train(self, batch_size=1, num_batches=100):
        ...
        for iter_batch in range(num_batches):
            ...
            batch_struct = self.dqn.singleBatch(batch, {'gamma': self.gamma}, update_node2vec_tag=1)
