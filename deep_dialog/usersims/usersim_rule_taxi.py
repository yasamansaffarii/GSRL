...
class RuleTaxiSimulator(UserSimulator):
    ...
    def _sample_action(self):
        ...
        if len(self.goal['inform_slots']) > 0:
            ...
            if 'pickup_location' in list(self.goal['inform_slots'].keys()): # 'taxiname' must appear in the first user turn
        ...
        for slot in list(user_action['inform_slots'].keys()):
            ...
            if slot_err_prob_sample < self.slot_err_probability: # add noise for slot level
                if self.slot_err_mode == 0: # replace the slot_value only
                    if slot in list(self.taxi_dict.keys()): user_action['inform_slots'][slot] = random.choice(self.taxi_dict[slot])
                    ...
                    if slot_err_random <= 0.33:
                        ...
                        random_slot = random.choice(list(self.taxi_dict.keys()))
    ...
    def response_thanks(self, system_action):
        ...
        if 'taxi' in list(system_action['inform_slots'].keys()):
    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in list(system_action['inform_slots'].keys()): # check all the constraints from agents with user goal
            ...
            if system_action['inform_slots']['taskcomplete'] == dialog_config.NO_VALUE_MATCH:
                ...
                if 'taxi' in list(self.state['request_slots'].keys()): del self.state['request_slots']['taxi']
